...
c       Debugging
c       ===============================================
c       Get the element error vectors that can be visualized in visit
c       For the velocity
        do e=1,nelv
          do i=1,nx*nx*nx
...
c       Write to file
        call outpost(errvecx,errvecy,
     &       errvecz,pr,t,'nrm')

        call outpost(truncvecx,truncvecy,
     &       truncvecz,pr,t,'crt')


c       Absolute errors
        call sub3(errvecx,vx_trc,vx,nxyze)
        call sub3(errvecy,vy_trc,vy,nxyze)
        call sub3(errvecz,vz_trc,vz,nxyze)

c       Write to file
        call outpost(errvecx,errvecy,
     &       errvecz,pr,t,'er1')

        call outpost(vx_hat_trc,vy_hat_trc,
     &       vz_hat_trc,pr_hat_trc,t,'hat')
        ...
        if (nio.eq.0) then
          ...
          do i=1,nx*nx*nx*2
            ...
            write(*,*) 'Vhat=', vz_hat(i,1,1,50),
     &                 'Vhat_trc=', vz_hat_trc(i,1,1,50)
...
c     sorting variables
      ...
      logical ifsort
      real umax,umin,rang,temprms,temppsnr
...
!! @note truncate and check the new error
      ...
      !now get the spectral space norm of err vect
       ...
       !now get the peak signal noise ratio
          temppsnr=20*log10(rang/tempnorm) 
