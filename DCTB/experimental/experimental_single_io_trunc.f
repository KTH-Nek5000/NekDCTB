...
c     Spectral error estimator
      real speindx(lelv),speindy(lelv),speindz(lelv),speindpr(lelv)
      real spsig(lelv)
      real specerrx(lx1,ly1,lz1,lelv)  
      real specerry(lx1,ly1,lz1,lelv)  
      real specerrz(lx1,ly1,lz1,lelv)  
...
c       =======================        
c       Calculate the spectral error indicators
c       =======================

        call speri_get(speindx,spsig,vx,lelv)
        call speri_get(speindy,spsig,vy,lelv)
        call speri_get(speindz,spsig,vz,lelv)
        do i= 1, lelv
          speindpr(i) = targeterr 
        enddo
...
c       Cast the data into single precision
        do i=1,nxyze
           vx_hat_trc_s(i,1,1,1)=vx_hat_trc(i,1,1,1)
           vy_hat_trc_s(i,1,1,1)=vy_hat_trc(i,1,1,1)
           vz_hat_trc_s(i,1,1,1)=vz_hat_trc(i,1,1,1)
           pr_hat_trc_s(i,1,1,1)=pm1(i,1,1,1)
        enddo

c        call copy(vx_hat_trc_s,vx_hat_trc,nxyze)
c        call copy(vy_hat_trc_s,vy_hat_trc,nxyze)
c        call copy(vz_hat_trc_s,vz_hat_trc,nxyze)
c        call copy(pr_hat_trc_s,pm1,nxyze)

        etime0 = dnekclock_sync()

        !Write the fields into a file
        call adios2_update(lglel,pr_hat_trc_s, vx_hat_trc_s, 
     &   vy_hat_trc_s, vz_hat_trc_s, t)
...
c    ================== For debugging ===============
        do e=1,nelv
          do i=1,lx1*lx1*lx1
            specerrx(i,1,1,e)=speindx(e)
            specerry(i,1,1,e)=speindy(e)
            specerrz(i,1,1,e)=speindz(e)
          enddo
        enddo

        call outpost(specerrx,specerry,specerrz,pr,t,'spe') 

        do e=1,nelv
          do i=1,lx1*lx1*lx1
            specerrx(i,1,1,e)=normv_physx(e)
            specerry(i,1,1,e)=normv_physy(e)
            specerrz(i,1,1,e)=normv_physz(e)
          enddo
        enddo

        call outpost(specerrx,specerry,specerrz,pr,t,'nrm') 
...
c       Adios writes the pressure in mesh 1, so read it in pm1 
        call adios2_read(lglelr,pr_hat_trc_s,vx_hat_trc_s,vy_hat_trc_s,
     &                   vz_hat_trc_s,
     &                   nvals,nelv,nelb,
...
c       Recast to double
        do i=1,nxyze
           vx_hat_trc(i,1,1,1)=vx_hat_trc_s(i,1,1,1)
           vy_hat_trc(i,1,1,1)=vy_hat_trc_s(i,1,1,1)
           vz_hat_trc(i,1,1,1)=vz_hat_trc_s(i,1,1,1)
           pm1(i,1,1,1)=pr_hat_trc_s(i,1,1,1)
        enddo
c        call copy(vx_hat_trc,vx_hat_trc_s,nxyze)
c        call copy(vy_hat_trc,vy_hat_trc_s,nxyze)
c        call copy(vz_hat_trc,vz_hat_trc_s,nxyze)
c        call copy(pm1,pr_hat_trc_s,nxyze)
...
c     work variables to apply the filter 
      ...
      integer uumax,vvmax,wwmax,nxyze           !monitoring variables
...
!entries until an error bound is met. THIS USES THE NORM OF THE ERROR
     ...
     &           kutv,v,spec_v,spec_vt,nx,targeterr,crs,ifsort,speind)
      implicit none
      include 'SIZE'    ! for size information such as lx1, etc ...
      include 'MASS'    ! for the mass matrix
      include 'WZ'
...
c     sorting variables
      ...
      logical ifsort
      real umax,umin,sig_rang,temprms,temppsnr
c     For spectral error indicator
      real speind(lelv)
...
c       compute the maximun and minimun values in ubase
        umax=-500
        umin=500
        do i=1,nxyz
            umax=MAX(umax,ubase(i,1,1,e))
            umin=MIN(umin,ubase(i,1,1,e))
        enddo
c       get the range of the elemnt
        sig_rang=abs(umax-umin)
...
!! @iterative procedure starts here
        ...
        do while (tempnorm.le.targeterr.and.kut.le.(nx-1)*3.and.
     &            temppsnr.ge.35)
...
!! @note truncate and check the new error
      ...
      !now get the spectral space norm of err vect
       ...
       !now get the peak signal noise ratio
          temppsnr=20*log10(sig_rang/tempnorm) 
...
!=======================================================================
!> @brief Apply operators to a given field
