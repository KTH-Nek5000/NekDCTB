...
but has the following aditions: 1. The path to the data compression tool
box must be specified.
``sh   export COMPRESS_SRC="$PWD/../../../DCTB"   export COMPRESS_INC=" -I"${COMPRESS_SRC}   COMPRESS_INC+=" -I"${COMPRESS_SRC}"/framework_files"``
2. The folders with the subroutines must be added to the shearch space
of the fortran compiler. ``sh   export FC="mpifort"${COMPRESS_INC}`` 3.
ADIOS2 instrumentation must be activated. This is done by including the
following flag in the preprocesor list. ``sh   export PPLIST="ADIOS2"``
...
4. Finally, some files from the toolbox folder must be copied into the
