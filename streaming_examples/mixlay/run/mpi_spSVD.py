...
##################### From here on can be in another file or class #######################
    ...
    def write_modes(self,U, lglel, first_mode, number_of_modes,prefix):
        ...
        for j in range(first_mode,first_mode+number_of_modes):
            for e in range(0,nel):
                ...
                if qoi != 3:
                    ...
                    dataout.elem[gl_e].vel[qoi,:,:,:]=np.reshape(U[e*nxyz:e*nxyz+nxyz,j],(data.lr1[2],data.lr1[1],data.lr1[0]))
                elif qoi ==3:
                    dataout.elem[gl_e].pres[0,:,:,:]=np.reshape(U[e*nxyz:e*nxyz+nxyz,j],(data.lr1[2],data.lr1[1],data.lr1[0]),order='F')
