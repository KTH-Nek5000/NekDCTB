...
#ifdef AMR
         IF (ISWEEP.EQ.1) THEN
            CALL col2(tmp1,amr_qmlt,ntot)
            CALL col2(tmp2,amr_qmlt,ntot)
            CALL DSOP(TMP1,'MXA',lx1,ly1,lz1)
            CALL DSOP(TMP2,'MXA',lx1,ly1,lz1)
            IF (IF3D) then
               CALL col2(tmp3,amr_qmlt,ntot)
               CALL DSOP(TMP3,'MXA',lx1,ly1,lz1)
            endif
         ELSE
            c = 1.e20

            call amr_fill_ch(tmp1,ntot,c)
            call amr_fill_ch(tmp2,ntot,c)
            if (if3d) call amr_fill_ch(tmp3,ntot,c)

            CALL DSOP(TMP1,'MNA',lx1,ly1,lz1)
            CALL DSOP(TMP2,'MNA',lx1,ly1,lz1)
            IF (IF3D) CALL DSOP(TMP3,'MNA',lx1,ly1,lz1)

            call h1_proj(tmp1,lx1,ly1,lz1)
            call h1_proj(tmp2,lx1,ly1,lz1)
            if (if3d) call h1_proj(tmp3,lx1,ly1,lz1)
         ENDIF
#else
         if (isweep.eq.1) then
...
#ifdef AMR
      call h1_proj(v1,lx1,ly1,lz1)
      call h1_proj(v2,lx1,ly1,lz1)
      if (if3d) call h1_proj(v3,lx1,ly1,lz1)
#endif
...
#ifdef AMR
         if (isweep.eq.1) then
            CALL col2(tmp,amr_qmlt,ntot)
            CALL DSOP(TMP,'MXA',lx1,ly1,lz1)
         else
            c = 1.e20
            call amr_fill_ch(tmp,ntot,c)
            CALL DSOP(TMP,'MNA',lx1,ly1,lz1)
            call h1_proj(tmp,lx1,ly1,lz1)
         endif
#else
...
#ifdef AMR
c-----------------------------------------------------------------------
      subroutine reset_obj()
      include 'SIZE'
      include 'INPUT'

      nobj = 0
      call izero(nmember,maxobj)
      call izero(object,maxobj*maxmbr*2)
