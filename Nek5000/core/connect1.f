...
#ifdef SLVRST
      ! this part should be done only once for AMR
      if (.not.AMR_IFRESET) then
#endif
C
...
C
      ...
      call setedge
#ifdef AMR
        ! mesh topology is checked in mesh manager, so some of the tests
        ! are run only when mesh is generated based on .re2
        ! skip this for mesh generated by mesh manager
...
#ifdef SLVRST
        ! destroy communicator if necessary
        if (AMR_IFRESET) call fgslib_gs_free (gsh_fld(1))
#endif
#ifdef AMR
        call amr_setupds(gsh_fld(1),lx1,ly1,lz1,nelv,nelgv,glo_num)
#else
         call get_vert
...
c
        ...
        ! destroy communicator if necessary
        if (AMR_IFRESET) then
            call fgslib_gs_free (gsh_fld(1))
            call fgslib_gs_free (gsh_fld(2))
        endif
#endif
#ifdef AMR
        call amr_setupds(gsh_fld(1),lx1,ly1,lz1,nelv,nelgv,glo_num)
        call amr_setupds(gsh_fld(2),lx1,ly1,lz1,nelt,nelgt,glo_num)
#else
         call get_vert
...
C========================================================================
...
#ifdef AMR
         call amr_zero_ch (vmult,lx1,ly1,lz1,lelt)
         call fgslib_gs_op(gsh_fld(ifield),vmult,1,1,0)
#else
...
#ifdef AMR
         call amr_zero_ch (tmult,lx1,ly1,lz1,lelt)
         call fgslib_gs_op(gsh_fld(ifield),tmult,1,1,0)
#else
...
#ifdef AMR
      ! define additional multiplicity array for correct setting of
      ! Dirichlet BC's on the children via DSOP.
      if (ifheat) then
         ifield = 2
         ntot = ntott
         nel = nelt
      ...
      call rone(amr_qmlt,ntot)
      call amr_zero_ch (amr_qmlt,lx1,ly1,lz1,lelt)
#endif
