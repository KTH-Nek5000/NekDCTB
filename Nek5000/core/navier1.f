...
C
         ...
         CALL LAGVEL
#ifdef AMR
         ! collect old pressure fields to simplify restart
         call amr_lagpr()
#endif
C
      ELSE
...
#ifdef AMR
      if (IFSTRS) then
        call amr_abort('Error: IFSTRS not supported by amr; opbinv')
      else
        if (ifield.eq.ifldmhd) then
          call amr_opdssum_msk(inp1,inp2,inp3,b1mask,b2mask,b3mask,
     $                         lx1,ly1,lz1,nelv)
        else
          call amr_opdssum_msk(inp1,inp2,inp3,v1mask,v2mask,v3mask,
     $                         lx1,ly1,lz1,nelv)
        endif
      endif
#else
...
#ifdef AMR
      ! make field continuous
      CALL h1_proj (out1,nx1,ny1,nz1)
      CALL h1_proj (out2,nx1,ny1,nz1)
      if (if3d) CALL h1_proj (out3,nx1,ny1,nz1)
#endif
...
#ifdef AMR
      if (IFSTRS) then
        call amr_abort('Error: IFSTRS not supported by amr; opbinv1')
      else
        if (ifield.eq.ifldmhd) then
          call amr_opdssum_msk(inp1,inp2,inp3,b1mask,b2mask,b3mask,
     $                         lx1,ly1,lz1,nelv)
...
#ifdef AMR
      if (IFSTRS) then
        call amr_abort('Error: IFSTRS not supported by amr; convuz')
      else
        if (ifield.eq.ifldmhd) then
          call amr_opdssum_msk(resv1,resv2,resv3,b1mask,b2mask,b3mask,
     $                         lx1,ly1,lz1,nelv)
        else
          call amr_opdssum_msk(resv1,resv2,resv3,v1mask,v2mask,v3mask,
     $                         lx1,ly1,lz1,nelv)
        endif
      endif
#else
