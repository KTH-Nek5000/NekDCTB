...
c
#ifdef AMR
      real sf(n,n),sft(n,n),atd(1),l(0:n)
#else
...
c     ibc = 2  <==>  Neumann,   
      ...
      do iface=1,2*ldim
         ...
         if (cbc(ied,e,ifield).eq.'s  ') ibc = 2
#ifndef AMR
         ! in current AMR implementation I don't mark faces this way, so it is not necessary
...
c
#ifdef AMR
      real dx(0:nz+2),ll,lm,lr,z(nz),y(1)
#else
...
c
#ifdef AMR
      real sf(n,n),sft(n,n),atd(1),l(0:n),y(1)
#else
...
c-----------------------------------------------------------------------
     ...
     $              , lrr(lelt),lrs(lelt),lrt(lelt)
      ...
      integer e
#ifdef AMR
      integer ce, cf  ! element and face number
      ...
      if (if3d) then
         ...
         ! that number of children sharing a parent face is 4
         scl = 0.25
         do e = 1, amr_mrtr_fcs_m
            cf = amr_nonc_fcs_f(e)
            ce = amr_nonc_fcs_e(e)
            call faces(l,scl,ce,cf,n,n,n)
         enddo
         call dsop(l,'sum',n,n,n)
#else
         call dssum(l,n,n,n)
#endif
         do e=1,nelv
...
c        call outmat(l(1,1,1,25),n,n,' L    ',25)
         ...
         do e = 1, amr_mrtr_fcs_m
            cf = amr_nonc_fcs_f(e)
            ce = amr_nonc_fcs_e(e)
            call faces(l,scl,ce,cf,n,n,1)
         enddo
         call dsop(l,'sum',n,n,1)
#else
         call dssum(l,n,n,1)
#endif
c        call outmat(l(1,1,1,25),n,n,' L    ',25)
