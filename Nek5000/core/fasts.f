...
c     Fill interiors
      ...
      call dface_ext    (v1)
#ifdef AMR
      call amr_dssum_sch(v1,lx1,ly1,lz1,nelv)
#else
...
c-----------------------------------------------------------------------
      ...
      call dface_ext(l)
#ifdef AMR
      ! Scale children values by 0.5 (0.25 in 3D). This is a hack, based on assumption
      ! that number of children sharing a parent edge is 2 (4 in 3D)
      scl = 0.5
      if (if3d) scl = 0.25
      do e = 1, amr_mrtr_fcs_m
         cf = amr_nonc_fcs_f(e)
         ce = amr_nonc_fcs_e(e)
         call faces(l,scl,ce,cf,lx1,ly1,lz1)
      enddo
      call dsop(l,'sum',lx1,lx1,lx1)
#else
