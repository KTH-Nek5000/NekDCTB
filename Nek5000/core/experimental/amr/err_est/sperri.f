...
!-----------------------------------------------------------------------
      ! stamp logs
      call amr_log(AMR_LP_PRD,'Spectral error indicator init.')
      ...
      if (SERI_NP.gt.SERI_NP_MAX) then
         call amr_abort('SERI_NP greater than SERI_NP_MAX.')
      endif
      il = SERI_NP+SERI_ELR
      jl = min(LX1,LY1)
      if (IF3D) jl = min(jl,LZ1)
      if (il.gt.jl) then
         call amr_abort('SERI_NP+SERI_ELR greater than L?1')
      endif
...
!-----------------------------------------------------------------------
      ...
      if (min(NX1,NY1).le.(SERI_NP+SERI_ELR)) then
        call amr_abort('Error: increase L[XYZ]1')
      endif
