...
#ifdef AMR
      ! I use full restart for pn-pn-2 so I need it here
      if_full_pres = .false.
#endif
...
#ifdef AMR
      ! original code can cause problems in AMR, as there is no
      ! warning about type of restart file used and swapping restarrt files
      ! between P_N-P_N and P_N-P_N-2 is possible and there are no warning
      !
      ! for now I disable swapping restarrt files between P_N-P_N and P_N-P_N-2
      if_full_pres = .false.
      if (.not.ifsplit) then
         if_full_pres = if_press_mesh
      elseif(if_press_mesh) then
         call amr_abort('Restart file missmatch.')
      endif
#else
      if_full_pres = .false.
...
c     if (ifmhd.and..not.ifdg) then   ! Current dg is for scalars only
      ...
      if (ifheat.and..not.ifdg) then  ! Don't project if using DG
         ...
         do ifield=3,nfield
#ifdef AMR
            call h1_proj (t(1,1,1,1,ifield-1),lx1,ly1,lz1)
#else
...
c     if (ifpert.and..not.ifdg) then ! Still not DG
      if (ifpert) then
         do jp=1,npert
            ifield = 1
#ifdef AMR
            call amr_oph1_proj(vxp(1,jp),vyp(1,jp),vzp(1,jp),
     $                         lx1,ly1,lz1,nelv)
#else
            ...
            if (.not.ifdg) then
               do ifield=2,nfield
                  call h1_proj(tp(1,ifield-1,jp),lx1,ly1,lz1)
               enddo
            endif
#else
            if (.not.ifdg) then
