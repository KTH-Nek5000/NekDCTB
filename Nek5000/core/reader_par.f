...
#ifdef AMR
      ! load mesh tree  and check if there are any refinement
      call amr_tree_load()
      if (AMR_IOSTART.eq.0) call read_re2_hdr(ifbswap, .true.)
#else
...
#ifdef AMR
      if (AMR_IOSTART.eq.0) call read_re2_data(ifbswap)
      call amr_mesh_init()
#else
...
#ifdef AMR
      call finiparser_getDbl(d_out,'amr:reflmax',ifnd)
      if (ifnd .eq. 1) then
         AMR_LMAX = abs(int(d_out))
      endif

      AMR_IOSTART = 0
      call finiparser_getDbl(d_out,'amr:restart',ifnd)
      if (ifnd .eq. 1) then
         AMR_IOSTART = abs(int(d_out))
      endif

      call finiparser_getBool(i_out,'amr:writeRefineRestart',ifnd)
      if(ifnd .eq. 1) then
        AMR_IOSAVE = .false.
        if(i_out .eq. 1) AMR_IOSAVE = .true.
      endif
#endif
...
#ifdef AMR
      call bcast(AMR_LMAX,isize)
      call bcast(AMR_IOSTART,isize)
      call bcast(AMR_IOSAVE,lsize)
#endif
...
#ifdef AMR
      if (AMR_IOSET_MAX.lt.AMR_IOSTART) then
         call amr_abort
     $    ('ERROR: AMR_IOSTART must be smaller or equal AMR_IOSET_MAX')
      endif

      if (ifmvbd)  call amr_abort
     $            ('Error: amv does not support moving boundaries')
      if (ifmoab)  call amr_abort
     $            ('Error: amr does not support moab')
      ...
      ! for now 2-level scheme is more efficient than multilevel scheme for nonconforming meshes
      param(43) = 1

      param(44) = 0
      !!!!! TO CHANGE !!! this part is not verbouse enough
#endif
