...
c      COMMON /SCRCG/ DUMM10(LX1,LY1,LZ1,LELT,1)
...
#ifdef AMR
      call amr_genwz        ! get interpolation parameters for octal interpolation
      if (int(param(43)).eq.0) call amr_genwz_hsmg   ! hsmg part

      call amr_set_noncon() ! set interpolation arrays

      ! this should be moved to other place
      call amr_gen_crs_basis()    ! initialise base functions for coarse grid solver
#endif
      ...
      if (AMR_IOSTART.eq.0) then
        ...
        ! load data from the restart file
        call amr_restart_read()
...
#ifdef AMR
      if (AMR_IOSTART.eq.0) call vrdsmsh          ! verify mesh topology
#else
...
c      endif
      ...
      call in_situ_end()
#ifdef DPROCMAP
      ! free mpi window used by dprocmap
