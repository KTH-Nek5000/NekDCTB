...
#ifdef AMR
      ! I repeat setup number of times, so I cannot use variable
      ! memory storage
      real a(lcr*lcr*lelv)
      ! this could be modifed to reduce memory footprint
      common /scrh1crs/ a
#else
...
c
#ifdef AMR
      call amr_set_vert(se_to_gcrs,ngv,nxc,nelv,.true.)
#else
      call get_vertex
...
#ifdef SLVRST
      ! destroy previous crs setup if necessary
      if (AMR_IFRESET) call fgslib_crs_free (xxth(ifield))
#endif
...
c
      ...
      do ie=1,nelv
#ifdef AMR
         call amr_apply_j_loc(uc(1,ie),amr_jmc_fc,amr_jmc_ed,
     $                        nxc,nyc,nzc,ie)
#endif
...
#ifdef AMR
         call amr_apply_jt_loc(uc(1,ie),amr_jmc_fc,amr_jmc_ed,
     $                        nxc,nyc,nzc,ie)
#endif
      enddo
c
...
c     Galerkin projection
      ...
      do j = 1,ncl
         do e = 1,nelv
#ifdef AMR
            call amr_get_crs_basis(w1(1,e),e,j)
#else
         ...
         do i = 1,ncl
#ifdef AMR
            call amr_get_crs_basis(b(1,i),e,i)
#endif
